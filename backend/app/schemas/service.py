...
class TimerConfiguration(BaseModel):
    ...
    on_calendar: Optional[str] = Field(
        None,
        description="OnCalendar timer specification (e.g., 'daily', '*-*-* 02:00:00')",
    )
    on_boot_sec: Optional[str] = Field(
        None, description="Run timer after system boot (e.g., '15min')"
    )
    on_startup_sec: Optional[str] = Field(
        None, description="Run timer after systemd startup (e.g., '30min')"
    )
    on_unit_active_sec: Optional[str] = Field(
        None,
        description="Run timer relative to when service was last active (e.g., '1hour')",
    )
    on_unit_inactive_sec: Optional[str] = Field(
        None,
        description="Run timer relative to when service was last inactive (e.g., '30min')",
    )
    accuracy_sec: Optional[str] = Field(
        "1min", description="Timer accuracy (default: 1min)"
    )
    randomized_delay_sec: Optional[str] = Field(
        None, description="Random delay to spread timer activations (e.g., '5min')"
    )
    persistent: bool = Field(
        False, description="Whether timer should be persistent across reboots"
    )
    wake_system: bool = Field(
        False, description="Whether timer should wake system from suspend"
    )

    # Convenience fields for common cron-like patterns
    cron_expression: Optional[str] = Field(
        None, description="Cron-style expression (will be converted to OnCalendar)"
    )
...
class EnhancedServiceCreateRequest(BaseModel):
    ...
    display_name: Optional[str] = Field(
        None, max_length=255, description="Human-readable display name"
    )
    description: Optional[str] = Field(None, description="Service description")

    # Systemd service configuration
    systemd_type: SystemdServiceType = Field(
        SystemdServiceType.SIMPLE, description="Systemd service type"
    )
    ...
    exec_reload: Optional[str] = Field(
        None, description="Command to reload the service"
    )
    exec_start_pre: Optional[str] = Field(
        None, description="Command to run before starting the service"
    )
    exec_start_post: Optional[str] = Field(
        None, description="Command to run after starting the service"
    )

    # Service behavior
    restart_policy: RestartPolicy = Field(
        RestartPolicy.ON_FAILURE, description="When to restart the service"
    )
    restart_sec: Optional[int] = Field(
        3, ge=0, description="Time to wait before restarting (seconds)"
    )
    timeout_start_sec: Optional[int] = Field(
        90, ge=1, description="Timeout for service start (seconds)"
    )
    timeout_stop_sec: Optional[int] = Field(
        90, ge=1, description="Timeout for service stop (seconds)"
    )
    ...
    working_directory: Optional[str] = Field(
        None, description="Working directory for the service"
    )
    ...
    # Dependencies
    after_units: Optional[List[str]] = Field(
        None, description="Units that should be started before this service"
    )
    before_units: Optional[List[str]] = Field(
        None, description="Units that should be started after this service"
    )
    wants_units: Optional[List[str]] = Field(
        None, description="Units that this service wants (weak dependency)"
    )
    requires_units: Optional[List[str]] = Field(
        None, description="Units that this service requires (strong dependency)"
    )
    conflicts_units: Optional[List[str]] = Field(
        None, description="Units that conflict with this service"
    )

    # Security and sandboxing
    no_new_privileges: Optional[bool] = Field(
        None, description="Disable privilege escalation"
    )
    private_tmp: Optional[bool] = Field(None, description="Use private /tmp directory")
    protect_system: Optional[str] = Field(
        None, description="Protect system directories (strict/yes/no)"
    )
    protect_home: Optional[bool] = Field(None, description="Protect /home directory")
    read_only_paths: Optional[List[str]] = Field(
        None, description="Paths to make read-only"
    )
    inaccessible_paths: Optional[List[str]] = Field(
        None, description="Paths to make inaccessible"
    )

    # Logging
    standard_output: Optional[str] = Field(
        "journal", description="Where to send stdout (journal/null/file:path)"
    )
    standard_error: Optional[str] = Field(
        "journal", description="Where to send stderr (journal/null/file:path)"
    )
    ...
    timer_config: Optional[TimerConfiguration] = Field(
        None, description="Timer configuration for scheduled services"
    )
    create_timer: bool = Field(
        False, description="Whether to create a timer unit for this service"
    )

    # Install section
    wanted_by: List[str] = Field(
        ["multi-user.target"], description="Targets that should include this service"
    )
    required_by: Optional[List[str]] = Field(
        None, description="Units that require this service"
    )
    also: Optional[List[str]] = Field(
        None, description="Additional units to enable/disable with this service"
    )

    # Management settings
    auto_start: bool = Field(
        True, description="Whether to automatically start the service after creation"
    )
    auto_enable: bool = Field(
        True, description="Whether to automatically enable the service after creation"
    )
    auto_restart: bool = Field(
        False, description="Enable automatic restart monitoring in Owleyes"
    )
    is_managed: bool = Field(
        True, description="Whether this service should be managed by Owleyes"
    )
    is_monitored: bool = Field(
        True, description="Whether this service should be monitored"
    )

    # Additional metadata
    tags: Optional[Dict[str, str]] = Field(
        None, description="Service tags for organization"
    )
    extra_data: Optional[Dict[str, Any]] = Field(
        None, description="Additional service metadata"
    )
...
class ServiceDeployRequest(BaseModel):
    ...
    service_config: EnhancedServiceCreateRequest = Field(
        ..., description="Service configuration"
    )
    dry_run: bool = Field(
        False, description="Validate configuration without creating the service"
    )
...
class ServiceDeployResponse(BaseModel):
    ...
    service_id: Optional[int] = Field(
        None, description="ID of the created service (if successful)"
    )
    service_name: str = Field(description="Name of the service")
    server_hostname: str = Field(description="Hostname of the target server")
    created_files: List[str] = Field(
        default_factory=list, description="List of files created on the server"
    )
    actions_performed: List[str] = Field(
        default_factory=list, description="List of actions performed"
    )
    timestamp: datetime = Field(description="When the deployment was performed")

    # Validation results (for dry_run)
    validation_errors: Optional[List[str]] = Field(
        None, description="Validation errors found during dry run"
    )
    systemd_files_preview: Optional[Dict[str, str]] = Field(
        None, description="Preview of generated systemd files"
    )
...
class ServiceTemplateResponse(BaseModel):
    ...
    service_config: EnhancedServiceCreateRequest = Field(
        description="Pre-configured service configuration"
    )
...
class ServiceValidationRequest(BaseModel):
    ...
    service_config: EnhancedServiceCreateRequest = Field(
        ..., description="Service configuration to validate"
    )
...
class ServiceValidationResponse(BaseModel):
    ...
    suggestions: List[str] = Field(
        default_factory=list, description="Configuration suggestions"
    )

    # Path validation results
    execution_path_exists: Optional[bool] = Field(
        None, description="Whether the execution path exists on the server"
    )
    working_directory_exists: Optional[bool] = Field(
        None, description="Whether the working directory exists"
    )
    user_exists: Optional[bool] = Field(
        None, description="Whether the specified user exists"
    )
    group_exists: Optional[bool] = Field(
        None, description="Whether the specified group exists"
    )

    # Dependency validation
    dependency_status: Optional[Dict[str, bool]] = Field(
        None, description="Status of required dependencies"
    )
...
class ServiceControlRequest(BaseModel):
    ...
    model_config = ConfigDict(json_schema_extra={"example": {"action": "restart"}})
...
class ServiceLogsRequest(BaseModel):
    ...
    since: Optional[str] = Field(
        None, description="Show logs since this time (e.g., '1 hour ago', '2025-01-01')"
    )
    until: Optional[str] = Field(None, description="Show logs until this time")
    follow: bool = Field(
        False, description="Follow log output (for WebSocket streaming)"
    )
    priority: Optional[LogLevel] = Field(
...
class ServiceResponse(BaseModel):
    ...
    server_hostname: str = Field(
        description="Hostname of the server this service runs on"
    )
...
class ServiceDiscoveryRequest(BaseModel):
    ...
    force_refresh: bool = Field(
        False, description="Force refresh even if recently discovered"
    )
    service_types: Optional[List[ServiceType]] = Field(
        None, description="Limit discovery to specific service types"
    )
...
class ServiceDiscoveryResponse(BaseModel):
    ...
    services_removed: int = Field(
        description="Number of services removed (no longer exist)"
    )
...
class ServiceStatsResponse(BaseModel):
    ...
    services_by_status: Dict[str, int] = Field(
        description="Count of services by status"
    )
    services_by_server: Dict[str, int] = Field(
        description="Count of services by server"
    )
    ...
    model_config = ConfigDict(
        json_schema_extra={
            "example": {
                ...
                "failed_services": 5,
                "services_by_status": {"active": 120, "inactive": 25, "failed": 5},
                "services_by_server": {
                    "web-server-01": 45,
                    "db-server-01": 30,
                    "api-server-01": 75,
...
class ServiceCreateRequest(BaseModel):
    ...
    restart_policy: str = Field(
        "no", description="Restart policy (no, always, on-failure)"
    )
...
class ServiceOverrideConfig(BaseModel):
    ...
    systemd_type: Optional[SystemdServiceType] = Field(
        None, description="Systemd service type"
    )
    ...
    exec_start_pre: Optional[str] = Field(
        None, description="Command to run before starting"
    )
    exec_start_post: Optional[str] = Field(
        None, description="Command to run after starting"
    )

    # Process and behavior overrides
    restart_policy: Optional[RestartPolicy] = Field(
        None, description="When to restart the service"
    )
    restart_sec: Optional[int] = Field(
        None, ge=0, description="Time to wait before restarting (seconds)"
    )
    timeout_start_sec: Optional[int] = Field(
        None, ge=1, description="Timeout for service start (seconds)"
    )
    timeout_stop_sec: Optional[int] = Field(
        None, ge=1, description="Timeout for service stop (seconds)"
    )
    ...
    # Logging overrides
    standard_output: Optional[str] = Field(
        None, description="Where to send stdout (journal/null/file:path)"
    )
    standard_error: Optional[str] = Field(
        None, description="Where to send stderr (journal/null/file:path)"
    )
    syslog_identifier: Optional[str] = Field(None, description="Syslog identifier")

    # Install section overrides
    wanted_by: Optional[List[str]] = Field(
        None, description="Targets that should include this service"
    )
    required_by: Optional[List[str]] = Field(
        None, description="Units that require this service"
    )
...
class ServiceUpdateRequest(BaseModel):
    ...
    override_config: Optional[ServiceOverrideConfig] = Field(
        None, description="Systemd override configuration"
    )

    # Timer configuration updates (for timer-based services)
    timer_config: Optional[TimerConfiguration] = Field(
        None, description="Timer configuration updates"
    )

    # Management settings
    auto_restart: Optional[bool] = Field(
        None, description="Enable automatic restart monitoring in Owleyes"
    )
    is_managed: Optional[bool] = Field(
        None, description="Whether this service should be managed by Owleyes"
    )
    is_monitored: Optional[bool] = Field(
        None, description="Whether this service should be monitored"
    )
    ...
    apply_immediately: bool = Field(
        True, description="Whether to apply changes immediately or stage them"
    )
    validate_only: bool = Field(
        False, description="Only validate changes without applying them"
    )
    create_backup: bool = Field(
        True,
        description="Create backup of current configuration before applying changes",
    )
    ...
    def validate_override_config(cls, v):
        ...
        if v and not any(
            ...
            raise ValueError(
                "Override configuration must contain at least one field to update"
            )
        return v
...
class ServiceUpdateResponse(BaseModel):
    ...
    override_file_path: Optional[str] = Field(
        None, description="Path to the created override file"
    )
    ...
    validation_warnings: Optional[List[str]] = Field(
        None, description="Validation warnings"
    )

    # Configuration preview
    override_content_preview: Optional[str] = Field(
        None, description="Preview of the override file content"
    )
    systemd_reload_required: bool = Field(
        False, description="Whether systemd daemon reload is required"
    )
    service_restart_required: bool = Field(
        False,
        description="Whether service restart is required for changes to take effect",
    )
...
class ServiceRollbackRequest(BaseModel):
    ...
    rollback_to_backup: bool = Field(
        True, description="Rollback to the most recent backup"
    )
    backup_file_path: Optional[str] = Field(
        None, description="Specific backup file to rollback to"
    )
    ...
    model_config = ConfigDict(
        json_schema_extra={
            "example": {"rollback_to_backup": True, "restart_service": True}
        }
    )
