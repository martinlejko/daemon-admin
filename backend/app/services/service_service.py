...
class ServiceService:
    ...
    async def get_service(
        ...
        query = (
            select(Service)
            .options(selectinload(Service.server))
    ...
    async def list_services(
        ...
        service_responses = [
            ServiceResponse.from_service(service) for service in services
        ]

        return service_responses, total

    async def control_service(
        self, db: AsyncSession, service_id: int, action: str
    ) -> ServiceControlResponse:
        ...
        try:
            success, message = await ssh_manager.control_service(
                service.server, service.name, action
            )
            ...
            logger.error(
                "Service control failed - SSH error",
                service_id=service_id,
                action=action,
                error=str(e),
            )
            ...
            logger.error(
                "Service control failed - unexpected error",
                service_id=service_id,
                action=action,
                error=str(e),
            )
    ...
    async def get_service_logs(
        ...
        try:
            success, logs = await ssh_manager.get_service_logs(
                service.server, service.name, lines, since, until, priority, grep
            )
            ...
            if success:
                ...
                logger.error(
                    "Failed to retrieve service logs", service_id=service_id, error=logs
                )
            ...
            error_msg = f"SSH connection failed: {str(e)}"
            logger.error(
                "Failed to retrieve service logs - SSH error",
                service_id=service_id,
                error=str(e),
            )
            ...
            logger.error(
                "Failed to retrieve service logs - unexpected error",
                service_id=service_id,
                error=str(e),
            )
    ...
    async def discover_services(
        self, db: AsyncSession, server_id: int, force_refresh: bool = False
    ) -> ServiceDiscoveryResponse:
        ...
        try:
            ...
            current_services_query = select(Service).filter(
            ...
            discovered_service_names = {
                service["name"] for service in discovered_services
            }
            ...
            for service in current_services:
                if (
                    service.name not in discovered_service_names
                    and service.service_type == ServiceType.SYSTEMD
                ):
            ...
            error_msg = f"SSH connection failed: {str(e)}"
            logger.error(
                "Service discovery failed - SSH error",
                server_id=server_id,
                error=str(e),
            )
            ...
            logger.error(
                "Service discovery failed - unexpected error",
                server_id=server_id,
                error=str(e),
            )
    ...
    async def get_service_stats(self, db: AsyncSession) -> ServiceStatsResponse:
        ...
        try:
            ...
            status_query = select(Service.status, func.count(Service.id)).group_by(
                Service.status
            )
            ...
            server_query = (
                select(Server.hostname, func.count(Service.id))
            ...
            timer_query = select(func.count(Service.id)).filter(
                Service.is_timer == True
            )
            ...
            return ServiceStatsResponse(
                ...
                inactive_services=services_by_status.get(
    ...
    async def _refresh_service_status(self, db: AsyncSession, service: Service) -> None:
        ...
        try:
            ...
            service_details = await ssh_manager._get_service_details(
                await ssh_manager.get_connection(service.server), service.name
            )
    ...
    async def validate_service_creation(
        ...
        try:
            ...
            if existing_service:
                errors.append(
                    f"Service {service_config.name} already exists in database"
                )
            ...
            try:
                ...
                is_valid, ssh_errors, ssh_warnings = (
                    await ssh_manager.validate_service_configuration(
                        server, service_dict
                    )
                )
            ...
            if warnings:
                suggestions.append(
                    "Review warnings to ensure optimal service configuration"
                )
            ...
            if not service_config.working_directory and service_config.exec_start:
                suggestions.append(
                    "Consider setting a working directory for better service isolation"
                )
    ...
    async def create_custom_service(
        ...
        try:
            ...
            try:
                success, message, created_files, systemd_files = (
                    await ssh_manager.create_systemd_service(
                ...
                new_service = Service(
                    ...
                    timer_schedule=(
                        service_config.timer_config.on_calendar
                        if service_config.timer_config
                        else None
                    ),
                    timer_file_path=(
                        f"/etc/systemd/system/{service_config.name}.timer"
                ...
                error_msg = f"SSH connection failed: {str(e)}"
                logger.error(
                    "Service creation failed - SSH error",
                    server_id=server_id,
                    service_name=service_config.name,
                    error=str(e),
                )
            ...
            logger.error(
                "Service creation failed - unexpected error",
                server_id=server_id,
                service_name=(
                    service_config.name
            ...
            return ServiceDeployResponse(
                ...
                server_hostname=(
                    server.hostname if "server" in locals() and server else "Unknown"
                ),
                created_files=[],
                actions_performed=[],
                timestamp=datetime.utcnow(),
    ...
    async def remove_custom_service(
        self, db: AsyncSession, service_id: int, remove_files: bool = True
    ) -> ServiceDeployResponse:
        ...
        try:
            ...
            if not service.is_custom_created:
                return ServiceDeployResponse(
                    ...
                    service_name=service.name,
                    server_hostname=(
                        service.server.hostname if service.server else "Unknown"
                    ),
                    created_files=[],
                    actions_performed=[],
                    timestamp=datetime.utcnow(),
            ...
            if remove_files and server.is_enabled:
                try:
                    ...
                    logger.warning(
                        "SSH connection failed during service removal",
                        service_id=service_id,
                        error=str(e),
                    )
            ...
            logger.info(
                "Custom service removed successfully",
                service_id=service_id,
                service_name=service.name,
                server_hostname=server.hostname,
    ...
    async def update_service(
        self, db: AsyncSession, service_id: int, update_request: ServiceUpdateRequest
    ) -> ServiceUpdateResponse:
        ...
        try:
            ...
            if update_request.override_config:
                try:
                    # Convert Pydantic model to dict for SSH manager
                    override_dict = update_request.override_config.model_dump(
                        exclude_none=True
                    )
                    ...
                    if update_request.validate_only:
                        is_valid, errors, warnings = (
                            await ssh_manager.validate_service_override(
                    ...
                    override_content_preview = (
                        ssh_manager._generate_override_file_content(override_dict)
                    )
                    ...
                    if update_request.apply_immediately:
                        success, message, override_path, backup_path = (
                            await ssh_manager.create_service_override(
                                service.server,
                                service.name,
                                override_dict,
                    ...
                    logger.error(
                        "Failed to handle override configuration",
                        service_id=service_id,
                        error=str(e),
                    )
                    return ServiceUpdateResponse(
            ...
            if update_request.auto_restart is not None:
                ...
                changes_applied.append(
                    f"{'Enabled' if update_request.auto_restart else 'Disabled'} auto-restart"
                )

            if update_request.is_managed is not None:
                service.is_managed = update_request.is_managed
                changes_applied.append(
                    f"{'Enabled' if update_request.is_managed else 'Disabled'} management"
                )

            if update_request.is_monitored is not None:
                service.is_monitored = update_request.is_monitored
                changes_applied.append(
                    f"{'Enabled' if update_request.is_monitored else 'Disabled'} monitoring"
                )
            ...
            if update_request.timer_config and service.is_timer:
                ...
                service.service_config["timer_config"] = (
                    update_request.timer_config.model_dump(exclude_none=True)
                )
                changes_applied.append(
                    "Updated timer configuration (requires manual timer restart)"
                )
            ...
            logger.error("Service update failed", service_id=service_id, error=str(e))
            ...
            return ServiceUpdateResponse(
                ...
                service_id=service_id,
                service_name=(
                    service.name if "service" in locals() and service else "Unknown"
                ),
                server_hostname=(
                    service.server.hostname
                    if "service" in locals() and service and service.server
                    else "Unknown"
                ),
                changes_applied=[],
                timestamp=datetime.utcnow(),
        ...
        try:
            ...
            if rollback_request.remove_override:
                ...
                success, message = await ssh_manager.remove_service_override(
                    service.server, service.name, remove_backup=False
                )
            ...
            if rollback_request.restart_service and service.is_managed:
                try:
                    ...
                    if success:
                        ...
                        logger.warning(
                            "Failed to restart service after rollback",
                            service_id=service_id,
            ...
            logger.info(
                "Service configuration rolled back successfully",
                service_id=service_id,
                service_name=service.name,
                server_hostname=service.server.hostname,
                changes_applied=changes_applied,
            ...
            logger.error("Service rollback failed", service_id=service_id, error=str(e))
