...
class ServerService:
    ...
    async def create_server(
        self, db: AsyncSession, server_data: ServerCreateRequest
    ) -> ServerResponse:
        """Create a new server."""
        try:
    ...
    async def update_server(
        self, db: AsyncSession, server_id: int, server_data: ServerUpdateRequest
    ) -> Optional[ServerResponse]:
        ...
        try:
            ...
            if "ssh_key_passphrase" in update_data:
                ...
                server.ssh_key_passphrase_encrypted = update_data.pop(
                    "ssh_key_passphrase"
                )
    ...
    async def gather_system_info(
        self, db: AsyncSession, server_id: int
    ) -> Optional[ServerSystemInfoResponse]:
        ...
        if not server.is_enabled:
            logger.warning(
                "Cannot gather system info from disabled server", server_id=server_id
            )
            return None
        ...
        try:
            ...
            logger.info(
                "System info gathered", server_id=server_id, hostname=server.hostname
            )
            ...
            logger.error(
                "Unexpected error gathering system info",
                server_id=server_id,
                error=str(e),
            )
            return None
        ...
        try:
            ...
            if success:
                ...
                try:
                    ...
                    logger.warning(
                        "Failed to gather system info during status update",
                        server_id=server.id,
                        error=str(e),
                    )
            ...
            logger.error(
                "Failed to update server status", server_id=server.id, error=str(e)
            )
