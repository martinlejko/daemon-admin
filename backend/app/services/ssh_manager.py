...
class SSHConnectionManager:
    ...
    def _generate_systemd_timer_file(self, service_name: str, timer_config: Dict[str, Any]) -> str:
        ...
        try:
            ...
            if exec_start:
                ...
                if command_parts:
                    ...
                    result = await self._execute_ssh_command(connection, check_cmd)
                    if not result.ok:
                        errors.append(f"Command not found or not executable: {command_path}")
            ...
            for dep_type in ['after_units', 'before_units', 'wants_units', 'requires_units']:
            ...
            logger.error("Failed to validate service configuration", 
                        hostname=server.hostname, 
                        error=str(e))
    ...
    async def create_systemd_service(
        ...
        try:
            ...
            if service_config.get('create_timer') and service_config.get('timer_config'):
                timer_file_content = self._generate_systemd_timer_file(service_name, service_config['timer_config'])
            ...
            if not result.ok:
                return False, f"Failed to create service file: {result.stderr}", [], systemd_files
            ...
            if timer_file_content:
                ...
                if not result.ok:
                    ...
                    await self._execute_ssh_command(connection, f"sudo rm -f {service_file_path}")
                    return False, f"Failed to create timer file: {result.stderr}", [], systemd_files
                created_files.append(timer_file_path)
            
            # Reload systemd daemon
            result = await self._execute_ssh_command(connection, "sudo systemctl daemon-reload")
            if not result.ok:
            ...
            if service_config.get('auto_enable', True):
                ...
                if not result.ok:
                    logger.warning("Failed to enable service", 
                                  hostname=server.hostname, 
                                  service=service_name,
                                  error=result.stderr)
            ...
            if service_config.get('auto_start', True):
                ...
                if not result.ok:
                    logger.warning("Failed to start service", 
                                  hostname=server.hostname, 
                                  service=service_name,
                                  error=result.stderr)
            ...
            return True, f"Service {service_name} created successfully", created_files, systemd_files
    ...
    async def remove_systemd_service(
        ...
        try:
            ...
            logger.info("Systemd service removed successfully", 
                       hostname=server.hostname,
                       service_name=service_name,
            ...
            logger.error("Systemd service removal failed", 
                        hostname=server.hostname, 
                        service_name=service_name,
                        error=str(e))
            return False, error_msg
    ...
    def _generate_override_file_content(self, override_config: Dict[str, Any]) -> str:
        ...
        if 'environment_variables' in override_config and override_config['environment_variables']:
        ...
        if 'inaccessible_paths' in override_config and override_config['inaccessible_paths']:
        ...
        for field, systemd_key in install_fields.items():
            if field in override_config and override_config[field] is not None:
                value = ' '.join(override_config[field]) if isinstance(override_config[field], list) else override_config[field]
    ...
    async def create_service_override(
        ...
        try:
            ...
            if not result.ok:
                return False, f"Failed to create override directory: {result.stderr}", override_file, backup_file
            ...
            if not result.ok:
                return False, f"Failed to write temporary override file: {result.stderr}", override_file, backup_file
            ...
            if not result.ok:
                ...
                return False, f"Failed to move override file to final location: {result.stderr}", override_file, backup_file
            
            # Set proper permissions
            chmod_cmd = f"sudo chmod 644 {override_file}"
            result = await self._execute_ssh_command(connection, chmod_cmd)
            if not result.ok:
                logger.warning("Failed to set override file permissions", 
                             hostname=server.hostname,
                             service=clean_service_name,
                             error=result.stderr)
            ...
            if not result.ok:
                return False, f"Failed to reload systemd daemon: {result.stderr}", override_file, backup_file
            
            logger.info("Service override created successfully",
                       hostname=server.hostname,
                       service_name=clean_service_name,
                       override_file=override_file,
                       backup_file=backup_file)
            
            return True, f"Override configuration applied successfully", override_file, backup_file
            
        except Exception as e:
            error_msg = f"Failed to create service override: {str(e)}"
            logger.error("Service override creation failed",
                        hostname=server.hostname,
                        service_name=service_name,
                        error=str(e))
    ...
    async def remove_service_override(
        ...
        try:
            ...
            if remove_backup:
                ...
                if not result.ok:
                    logger.warning("Failed to remove backup files",
                                 hostname=server.hostname,
                                 service=clean_service_name,
                                 error=result.stderr)
            
            # Check if override directory is empty and remove it
            check_empty_cmd = f"[ -d {override_dir} ] && [ -z \"$(ls -A {override_dir})\" ]"
            ...
            logger.info("Service override removed successfully",
                       hostname=server.hostname,
                       service_name=clean_service_name,
            ...
            logger.error("Service override removal failed",
                        hostname=server.hostname,
                        service_name=service_name,
                        error=str(e))
            return False, error_msg
    ...
    async def validate_service_override(
        ...
        try:
            ...
            check_service_cmd = f"systemctl list-unit-files {clean_service_name}.service"
            ...
            if not result.ok or clean_service_name not in result.stdout:
                errors.append(f"Service {clean_service_name} does not exist on the server")
            ...
            if 'user' in override_config and override_config['user']:
                ...
                if not result.ok:
                    errors.append(f"User '{override_config['user']}' does not exist on the server")
            ...
            if 'group' in override_config and override_config['group']:
                ...
                if not result.ok:
                    errors.append(f"Group '{override_config['group']}' does not exist on the server")
            
            # Validate working directory if specified
            if 'working_directory' in override_config and override_config['working_directory']:
                dir_check_cmd = f"test -d {override_config['working_directory']}"
                result = await self._execute_ssh_command(connection, dir_check_cmd)
                if not result.ok:
                    warnings.append(f"Working directory '{override_config['working_directory']}' does not exist")
            
            # Validate executable paths in exec commands
            exec_fields = ['exec_start', 'exec_stop', 'exec_reload', 'exec_start_pre', 'exec_start_post']
                ...
                if field in override_config and override_config[field]:
                    ...
                    if command:
                        ...
                        if executable.startswith('/'):
                            ...
                            result = await self._execute_ssh_command(connection, exec_check_cmd)
                            if not result.ok:
                                warnings.append(f"Executable '{executable}' in {field} does not exist or is not executable")
            ...
            if 'environment_file' in override_config and override_config['environment_file']:
                ...
                if not result.ok:
                    warnings.append(f"Environment file '{override_config['environment_file']}' does not exist")
            
            # Validate unit dependencies
            dependency_fields = ['after_units', 'before_units', 'wants_units', 'requires_units']
            for field in dependency_fields:
                if field in override_config and override_config[field]:
                    for unit in override_config[field]:
                        unit_check_cmd = f"systemctl list-unit-files {unit}"
                        result = await self._execute_ssh_command(connection, unit_check_cmd)
                        if not result.ok:
                            warnings.append(f"Dependency unit '{unit}' in {field} may not exist")
            
            is_valid = len(errors) == 0
            
            logger.info("Service override validation completed",
                       hostname=server.hostname,
                       service_name=clean_service_name,
                       is_valid=is_valid,
            ...
            logger.error("Service override validation failed",
                        hostname=server.hostname,
                        service_name=service_name,
                        error=str(e))
    ...
    async def get_service_override_content(
        ...
        try:
            ...
            override_file = f"/etc/systemd/system/{clean_service_name}.service.d/override.conf"
            ...
            logger.error("Failed to read service override content",
                        hostname=server.hostname,
                        service_name=service_name,
                        error=str(e))
